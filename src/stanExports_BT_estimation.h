...
namespace model_BT_estimation_namespace {
using std::istream;
using std::string;
...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "model_BT_estimation");
    reader.add_event(38, 36, "end", "model_BT_estimation");
    return reader;
}
#include <stan_meta_header.hpp>
class model_BT_estimation
  : public stan::model::model_base_crtp<model_BT_estimation> {
private:
        int N;
        int nrt;
        std::vector<int> BCM;
...
public:
    model_BT_estimation(stan::io::var_context& context__,
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        static const char* function__ = "model_BT_estimation_namespace::model_BT_estimation";
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        current_statement_begin__ = 13;
        if (!(context__.contains_r("BT")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable BT missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("BT");
        pos__ = 0U;
        validate_non_negative_index("BT", "nrt", nrt);
        context__.validate_dims("parameter initialization", "BT", "double", context__.to_vec(nrt));
        std::vector<double> BT(nrt, double(0));
        ...
        for (size_t i_0__ = 0; i_0__ < BT_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_lub_unconstrain(0.1, 1, BT[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable BT: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
    ...
    T__ log_prob(std::vector<T__>& params_r__,
        ...
        try {
            ...
            current_statement_begin__ = 13;
            std::vector<local_scalar_t__> BT;
            size_t BT_d_0_max__ = nrt;
            BT.reserve(BT_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < BT_d_0_max__; ++d_0__) {
                if (jacobian__)
                    BT.push_back(in__.scalar_lub_constrain(0.1, 1, lp__));
                else
                    BT.push_back(in__.scalar_lub_constrain(0.1, 1));
            }
            // model body
            {
    ...
    void write_array(RNG& base_rng__,
        ...
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
        static const char* function__ = "model_BT_estimation_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        for (size_t k_0__ = 0; k_0__ < BT_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "BT" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        if (!include_gqs__ && !include_tparams__) return;
...
typedef model_BT_estimation_namespace::model_BT_estimation stan_model;
#ifndef USING_R
